	...
	public static class HighleyGate
	{
		...
		if(!p1.exists())
		{
			try
			{
				...
				writer.close();
				p1.createNewFile();
				System.out.println("Le fichier " + p1 + " Vient d'�tre g�n�rer � l'emplacement : " + location);
			}
			catch (IOException e)
			{
				e.printStackTrace();
		...
		if(!p2.exists())
		{
			try
			{
				...
				writer.close();
				p2.createNewFile();
				System.out.println("Le fichier " + p2 + " Vient d'�tre g�n�rer � l'emplacement : " + location);
			}
			catch (IOException e)
			{
				e.printStackTrace();
